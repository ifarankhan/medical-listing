...
const randomData = function (callback) {
    let chartData = [];
    // Call AJAX to fetch message (Leads) / Months.
    $.ajax({
        url: '/customer-queries-per-month', // Route for your data
        method: 'GET',
        success: function (response) {
            // Assuming the response is an array of objects with `month` and `message_count`
            response.forEach(function (data) {
                chartData.push({
                    key: String(data.month).padStart(2, '0'), // Format month as 01, 02, etc.
                    value: data.message_count
                });
            });

            // Call the callback function after the data is ready
            callback(chartData);
        },
...
// Initialize the chart with data after the AJAX call completes
randomData(function (chartData) {
    renderChart(chartData);
});

// Bind the click event to refresh the chart when the button is clicked
...
function renderChart(chartData) {
    $(".example-two").simpleBarGraph({
        ...
        barsColor: "#3D9970",
        stepSize: 1,            // Ensure step size is 1 to avoid decimals

    });
}

// Add custom labels for X and Y axes outside of the chart container
if (!$('.x-axis-label').length) {
    // Add X-axis label below the chart container
    $('.example-two').after(`<div class="x-axis-label">Months</div>`);
}

if (!$('.y-axis-label').length) {
    // Add Y-axis label to the left of the chart container
    $('.example-two').before(`<div class="y-axis-label">Number of Messages</div>`);
}

// Apply CSS styles for the axis labels
$('.x-axis-label').css({
    'text-align': 'center',
    'margin-top': '10px',
    'font-weight': 'bold'
...
$('.y-axis-label').css({
    'position': 'absolute',
   // 'left': '0', // Adjust this to ensure it's visible
    'top': '45%',
    'transform': 'rotate(-90deg)',
    'transform-origin': 'left center', // Ensure proper alignment
    'font-weight': 'bold',
    'white-space': 'nowrap', // Prevent text from wrapping
    'margin-left': '-15px' // Fine-tune positioning to avoid clipping
});
